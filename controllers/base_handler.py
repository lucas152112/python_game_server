...
from base import error, const, commands_s2s
...
REFUSE_TIME_OUT_REQUEST_SECOND = 20
...
class BaseHandler(tornado.web.RequestHandler):
    ...
    def __publish_message(obj):
        ...
        s.connect((config.get_item('router_ip'), config.get_item('router_port')))
    ...
    @staticmethod
    def share_mongo():
        db_mongo_info = config.get_item("mongo")
        default_conn = db_mongo_info['alias']
        db_mongo = get_db(default_conn)
        return db_mongo
    ...
    # 回答客户端
    def write_json(self, status, data=None, depth=1):
        desc = sys._getframe(depth).f_lineno if status != error.OK else ""
        self._result = {'status': status, 'desc': desc, 'data': data or []}
        self.write(self._result)
        self.finish()
        return

    # def write_error(self, code, desc=None, data=None):
    #     desc = sys._getframe(1).f_lineno if not desc else desc
    #     self._result = {'status': code, 'desc': desc, 'data': data or []}
    #     self.write(self._result)
    #     self.finish()
    #     return
    ...
    #
    #     now = utils.timestamp()
    #     if now > self.get_int("time") + REFUSE_TIME_OUT_REQUEST_SECOND:
    #         return False
    #
    #     return sign_passed
    ...
    def setup_fixed_params(self):
        ...
        # if self.uid:
        #     online_model.update_player_activity(self.uid)
    ...
    def write_error(self, status_code, **kwargs):
        if status_code == 500:
            ...
            database.share_redis_game().publish('exceptions', errors)
