...
def _check_daemon(server_class, server_name, service_type=-1, is_gate=True):
    if not issubclass(server_class, BaseService):
        return
    ...
    if not server_id:
        if service_type == const.SERVICE_GATE:
            server_id = 1
        else:
            return -1, False
...
def start(server_class, service_type, server_name, is_gate=True):
    assert server_class
    assert server_name
    assert service_type
...
def start_gate(server_class, service_type, server_name):
    utils.log("server_class:" + str(server_class),"gate.log")
    utils.log("service_type:" + str(service_type),"gate.log")
    utils.log("server_name:" + server_name,"gate.log")
    server_name = server_name.split("/")[-1]
    server_id, is_daemon = _check_daemon(server_class, server_name, service_type)
    utils.log("server_id:" + server_id,"gate.log")
    _init_daemon(server_class, server_name, server_id, is_daemon)
